#!/usr/bin/env python3
""" country_converter_ - Classification converter for countries"""
...
from country_converter_.version import __version__
    ...
    def convert(
        ...
        for ind_names, current_name in enumerate(names):
            ...
            if src_format.lower() in ["regex", "iso2"]:
                ...
                for ind_regex, ccregex in enumerate(regexes):
                    if ccregex.search(spec_name):
                        removelist[ind_names] = ccregex.sub(' ', removelist[ind_names])
            ...
            if len(result_list) == 0:
                ...
                removelist[ind_names] = current_name
            else:
                outlist[ind_names] = []
        ...
        if (len(outlist) == 1) and not enforce_list:
            return outlist[0], removelist[0]
        else:
            return outlist, removelist

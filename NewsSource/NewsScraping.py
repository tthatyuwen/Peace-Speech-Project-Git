...
def get_text(article_url_list):                               # use aiohttp and request to get text,perhaps faster
    async def get_source(url):
        async with aiohttp.ClientSession() as session:
            async with await session.get(url,headers=headers,timeout=3) as response:
...
def get_url(driver,param):
    ...
    for li in li_list:
        try:
            ...
            pass
    if len(article_url_list)>30:
        article_url_list = random.sample(article_url_list, 30)
...
def get_country_param(country,startdate,page=1):          # setting for different websites
    ...
    params = {
        ...
        'Norway': {
            ...
            'list_path': '//h3[@class="entry-title td-module-title"]',
            'detailed_path': './a/@href'},
        'Uganda': {
        ...
        "Sri_Lanka":{
            ...
            'detailed_path':"./a/@href"},
        "UK":{
            'url':f'https://www.independent.co.uk/archive/{startdate.year}-{startdate.strftime("%m")}-{startdate.strftime("%d")}',
            'list_path':'//*[@id="frameInner"]//li',
            'detailed_path':"./a/@href"},
        "Canada":{
            'url':f'https://www.thestar.com/archive/{startdate.year}/{startdate.strftime("%m")}/{startdate.strftime("%d")}.html',
...
def main(country,startdate,setting_params={}):
    if len(setting_params)==0:
        setting_params = get_country_param(country,startdate)
        article_url_list = get_url(set_driver(), setting_params)
    if country =='Nigeria':
        article_url_list = ['https://www.thenigerianvoice.com/'+'amp/'+url.split('//')[-1] for url in article_url_list]
    if country == 'Nigeria' or 'Sri_Lanka':
...
def Uganda_news(page):
    article_url_list = get_url(set_driver(), get_country_param('Uganda',date.today(),page))
    driver = set_driver()
    ...
    for url in article_url_list:
        try:
            ...
            f = open(path, 'a+')     # dataformat shall be modified later
            try:
